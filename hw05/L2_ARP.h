...
#define DEFAULT_MAC_ADDR	"ff:ff:ff:ff:ff:ff"
...
typedef struct PacketData_name {
	byte* data;
	uint64_t dataLen;
	string ip;
} PacketData;
typedef struct CacheLine_name {
	char* MACAddress;
	bool isValid;
	uint64_t genesis;
} CacheLine;
...
private:
	bool debug;
	NIC* nic;
	map<string, PacketData> pktQueue;
	map<string, CacheLine> ARPCache;
};
